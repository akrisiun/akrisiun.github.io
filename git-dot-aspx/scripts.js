...
var anchorSplitChar = ",";
var folder = "/git-dot-aspx";
// http://localhost:?/folder/
var host = "http://localhost" + folder;
...
function onPageLoaded() {
    ...
    if (query && query.slice(0, 3) == "?q=") {
        redirectLocation(top, folder + "/#" + query.slice(1));
        return;
    }

    var pathname = document.location.pathname;
    if (pathname.length > 11 + folder.length && pathname.toLowerCase().slice(folder.length, 11) == "/index.html") {
        redirectLocation(top, folder + "/");
        return;
    }
...
function setHash(newHash) {
    ...
    if (folder.length > 1 && newHash.length > folder.length)
    {
        ...
        if (slice0 == folder)
            newHash = newHash.substr(folder.length);
    }
...
function processHash() {
    ...
    if (anchor) {
        ...
        if (!anchor) {
            if (top.location.pathname != folder + "/") {
				alert(folder + "/");
                // redirectLocation(top, folder + "/");
            }
            return;
        ...
        if (startsWithIgnoreCase(anchor, "MSBuildProperty=")) {
            redirectLocation(n, folder + "/MSBuildProperties/R/" + anchor.slice("MSBuildProperty=".length) + ".html");
            return;
        }

        if (startsWithIgnoreCase(anchor, "MSBuildItem=")) {
            redirectLocation(n, folder + "/MSBuildItems/R/" + anchor.slice("MSBuildItem=".length) + ".html");
            return;
        }

        if (startsWithIgnoreCase(anchor, "MSBuildTarget=")) {
            redirectLocation(n, folder + "/MSBuildTargets/R/" + anchor.slice("MSBuildTarget=".length) + ".html");
            return;
        }

        if (startsWithIgnoreCase(anchor, "MSBuildTask=")) {
            redirectLocation(n, folder + "/MSBuildTasks/R/" + anchor.slice("MSBuildTask=".length) + ".html");
            return;
        }

        if (startsWithIgnoreCase(anchor, "EmptyArrayAllocation")) {
            redirectLocation(n, folder + "/mscorlib/R/EmptyArrayAllocation.html");
            return;
        }
        ...
        if (potentialFile.charAt(0) == folder + "/") {
            potentialFile = potentialFile.slice(1);
        }

        if (potentialFile.charAt(potentialFile.length - 1) == folder + "/") {
        ...
        if (isFile(potentialFile)) {
            ...
            var pathParts = potentialFile.split(folder + "/");
            if (pathParts.length > 1) {
                if (hashParts.length == 3 && hashParts[2] == "references") {
                    redirectLocation(n, folder + "/" + pathParts[0] + "/R/" + hashParts[1] + ".html");
                }
                else {
                    if (pathParts[0] != "MSBuildFiles" && pathParts[0] != "TypeScriptFiles") {
                        redirectLocation(n, folder + "/" + pathParts[0] + "/ProjectExplorer.html");
                    }
                }
            }
        } else if (hashParts.length == 1 && potentialFile.indexOf("/") == -1) {
            redirectLocation(n, folder + "/" + potentialFile + "/ProjectExplorer.html");
        } else if (hashParts.length == 2 && potentialFile.indexOf("/") == -1 && hashParts[1] == "namespaces") {
            redirectLocation(n, folder + "/" + potentialFile + "/namespaces.html");
        }
    ...
    if (!isLargeFile) {
        ...
        if (projectPathLink) {
            projectPathLink.onclick = function () {
                ...
                top.n.location = folder + "/" + assemblyName + "/ProjectExplorer.html";
...
function ro() {
    ...
    if (startsWithIgnoreCase(path, folder + "/MSBuildProperties")) {
        newHash = "MSBuildProperty=" + symbolId;
    } else if (startsWithIgnoreCase(path, folder + "/MSBuildItems")) {
        newHash = "MSBuildItem=" + symbolId;
    } else if (startsWithIgnoreCase(path, folder + "/MSBuildTargets")) {
        newHash = "MSBuildTarget=" + symbolId;
    } else if (startsWithIgnoreCase(path, folder + "/MSBuildTasks")) {
        newHash = "MSBuildTask=" + symbolId;
    } else if (startsWithIgnoreCase(path, folder + "/mscorlib/R/EmptyArrayAllocation")) {
...
function parseTabChannel(div) {
    ...
    for(var i = 0; i < aList.length; i++)
    {
        ...
        if (pos >= 0)
            a.href = folder + a.href.substr(pos);
    }
}

function parseProjectImg(div) {

    for (var i = 0; i < div.children.length; i++) {
        var child = div.children[i];
        if (child.innerHTML.indexOf("undefined") > 0) {
            var img1 = child.firstChild;
            if (img1.tagName == "IMG")
            {
                var pos = img1.src.indexOf("undefined");
                if (pos > 0)
                    img1.src = folder + img1.src.substr(pos + 9);
            }
        }

        if (child.innerHTML.indexOf('href="/#') > 0) {
            var elem = child.getElementsByTagName("a");
            if (elem) { // typeof ? !== "undefined") 
                for (var j = 0; j < elem.length; j++) {
                    a = elem[j];
                    if (a.tagName == "A" && a.href.indexOf("#") > 0) {
                        var href = a.href;
                        a.href = folder + href.substr(href.indexOf("#") - 1);
                    }
                }
            }
        }
        ...
        if (child.innerHTML.indexOf('href="/') > 0) {
            ...
            if (elem) {
                for (var j = 0; j < elem.length; j++) {
                    a = elem[j];
                    if (a.tagName == "A" && a.pathname.indexOf("/") == 0
                        && !a.pathname.startsWith(folder)
                        && a.href.indexOf("@#") < 0)
                    {
                        var href = a.pathname;
                        a.href = folder + href + a.hash;
                    }
                }
            }
        }
...
function initializeProjectExplorerRootFolder() {
    ...
    if (rootFolder) {
        var channelTab = rootFolder.previousElementSibling;
        if (channelTab)
            parseTabChannel(channelTab);
...
function initializeNamespaceExplorer() {
    makeFoldersCollapsible(/* namespace */"90.png", "90.png", 
		folder + "/content/icons/", /*initializeSolutionExplorerFolder:*/ null);

    var tabChannel = document.getElementsByClassName("tabChannel");
    if (tabChannel && tabChannel.length > 0)
        parseTabChannel(tabChannel[0]);
}
...
function makeFoldersCollapsible(folderIcon, openFolderIcon, pathToIcons, initializeHandler) {
    ...
    for (var i = 0; i < length; i++) {
        var folder1 = elements[i];
        folder1.style.display = 'none';
        folder1.initialize = initializeHandler;
        var div = folder1.previousSibling;
        ...
        var handler = expandCollapseFolder(folder1, imagePlusMinus, imageFolder, div, firstChild,
			pathToIcons, folderIcon, openFolderIcon);
...
function expandCollapseFolder(capturedFolder, capturedPlusMinus, capturedFolderImage, capturedDiv, capturedFirstChild, pathToIcons, folderIcon, openFolderIcon) 
{
    ...
    return function () {
        if (capturedFolder.style.display == 'none') {
            ...
            parseProjectImg(capturedFolder);
